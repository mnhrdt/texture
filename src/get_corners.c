...
// output  : xmin ymin W H rectangle to crop from the huge image
//
...
int main_get_corners(int c, char *v[])
{
    ...
    // for loop on corners of dsm image to get projections on huge_image
    for (int j = 0; j < 2; j++)
        for (int i = 0; i < 2; i++)
        {
